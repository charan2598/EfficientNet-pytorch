...
class ClassificationTrianer:
    def __init__(
        ...
        self.learning_rate_scheduler = (
            ...
            else self.raise_exception("LearningRate Scheduler is None")
        )
        self.loss_function = (
    ...
    def step_learning_rate_scheduler(self, validation_loss):   
        if self.learning_rate_scheduler is not None:
            if isinstance(self.learning_rate_scheduler, lr_scheduler.ReduceLROnPlateau):
                self.learning_rate_scheduler.step(validation_loss)
            else:
    ...
    def train(self):
        ...
        for epoch in progress_bar:
            ...
            current_learning_rate = self.step_learning_rate_scheduler(validation_loss)
            progress_bar.set_description(
                f"Epoch: {epoch+1}/{self.num_epochs} Learning Rate: {current_learning_rate} "
                + f"Training Loss: {train_loss} Validation Loss: {validation_loss} "
                + f"Training Accuracy: {train_accuracy} Validation Accuracy {validation_accuracy}"
            )
